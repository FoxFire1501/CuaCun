...
async function clonelCommand(message: Message<true>, channel?: string, ...args: string[]) {
    ...
    if (newChannel) (await (await newChannel.fetch()).guild.channels.cache.get(newChannel.id)?.fetch() as TextChannel).send({
        content: `Cloned by \`\`${message.author.username}\`\``
    })
