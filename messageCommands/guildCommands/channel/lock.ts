import { Guild, GuildChannel, Message, PermissionFlagsBits, userMention } from "discord.js";
...
async function lockChannelCommand(message: Message<true>, channel?: string, ...args: string[]) {
    ...
    message.reply({
        embeds: [
            {
                description: `${config.emojis.success} **Đã lock channel <#${channelTarget?.id}> thành công**`,
