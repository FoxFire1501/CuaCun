...
async function kickCommand(message: Message<true>, user?: string, ...args: string[]) {
    ...
    message.reply({
        embeds: [
            {
                description: `${config.emojis.success} **Đã Kick ${target.user.tag} thành công** | ${reason ? reason : "Không có lý do!"}`,
            ...
            embeds: [
                {
                    ...
                    description: `Bạn bị kick bởi ${userMention(message.member.user.id)}`,
